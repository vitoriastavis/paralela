...
{	
	...
	{	
		for(int j = 2; j < size_b + 1; j++)
		{
			if (seq_b[j - 2] == c_recv[i]) 
			{		
				p_recv[(i * (size_b + 1)) + j] = j - 1;		
			}
			else
			{				
...
int lcs_mpi(int size_a, int size_b, int size_c, mtype** s_matrix, mtype* p_matrix, char* seq_a, char* seq_b, char* seq_c, int rank, int chunk_size)
{
    ...
    for (int i = 1; i < size_b + 1; i++)
    {
		...
		{		
			...
			{
				// printf("if 1 \n");
				s_recv[j - start_id] = s_matrix[i - 1][j - 1] + 1;
			}
			else if(p_matrix[(c * (size_a+1)) + j] == 0)
			{
				// printf("if 2 \n");
				s_recv[j - start_id] = max(s_matrix[i - 1][j], 0);				
			}
			else
			{
				// printf("if 3 \n");
				s_recv[j - start_id] = max(s_matrix[i - 1][j], s_matrix[i - 1][p_matrix[(c * (size_a+1))+j]-1]+1);
				// printf("FPOI AQUOI \n");
			}
		}	
...
int main(int argc, char ** argv)
{
	...
	*/
	...
	if(rank == 0)
	{
		printf("chunk_p: %d; chunk_dp: %d; rank: %d; procs: %d\n", chunk_size_p, chunk_size_s, rank, num_procs);
	}
