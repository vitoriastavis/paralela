/*
    ...
    versao que mais faz sentido pra mim mas não funciona
	versão sem os scatter/b, tentando aplicar o chunk_size certo
*/
...
void calc_P_matrix(mtype *P, char *b, int len_b, char *c, int len_c, int rank, int chunk_size)
{

    char c_recv[chunk_size];
    mtype p_recv[chunk_size][len_b+1];  

    int start = rank * chunk_size;
    int end = min(start + chunk_size, len_c);    
    ...
    MPI_Allgather(p_recv, chunk_size*(len_b+1), MPI_UNSIGNED_SHORT, P, chunk_size*(len_b+1), MPI_UNSIGNED_SHORT, MPI_COMM_WORLD);
}
...
int main(int argc, char ** argv)
{
    ...
    // calculo tamanho dos chunks para as threads
	chunk_size_p = (len_c + num_procs - 1)/num_procs;
    chunk_size_s = ((len_b + num_procs) / num_procs);
	...
	p_matrix = (mtype *)malloc((len_c * (len_b + 1)) * sizeof(mtype));
